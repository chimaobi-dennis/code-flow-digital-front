...
export const Hero = ({ onCtaClick }: HeroProps) => {
  ...
  return <section className={`min-h-screen flex items-center pt-20 relative overflow-hidden bg-gradient-to-br ${currentSlideData.bgGradient} transition-all duration-1000`}>
      {/* Floating Bubbles */}
      ...
      {/* Navigation Arrows */}
      <button onClick={prevSlide} className="absolute left-8 top-1/2 -translate-y-1/2 z-20 w-12 h-12 bg-white/80 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-white hover:scale-110 transition-all duration-300 shadow-lg">
        <ChevronLeft className="w-6 h-6 text-gray-700" />
      </button>
      
      <button onClick={nextSlide} className="absolute right-8 top-1/2 -translate-y-1/2 z-20 w-12 h-12 bg-white/80 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-white hover:scale-110 transition-all duration-300 shadow-lg">
        ...
        <div className="text-center max-w-5xl mx-auto">
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="flex items-center justify-center gap-3 pt-12">
              {SLIDES.map((_, index) => <button key={index} onClick={() => goToSlide(index)} className={`w-3 h-3 rounded-full transition-all duration-500 ${index === currentSlide ? 'bg-gradient-to-r from-primary to-accent scale-125' : 'bg-gray-300 hover:bg-gray-400 hover:scale-110'}`} />)}
            </div>
